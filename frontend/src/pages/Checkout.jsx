...
import { payAndShip } from '../services/paymentApi'
...
export default function Checkout() {
  ...
  // Secure payment: the backend creates the Razorpay order, opens the modal,
  // verifies the signature server-side, and books shipping. The secret key
  // never touches the browser. (See src/services/paymentApi.js)
  const pay = async () => {
    if (!validate()) { window.scrollTo(0,0); return }
    setLoading(true)
    try {
      const result = await payAndShip({
        amountInr: grand,
        receipt: `BB${Date.now()}`,
        customer: {
          name: form.name,
      ...
      // Payment verified. (result.shipment has the courier + AWB if Bigship is configured.)
      clearCart()
      ...
      // Razorpay cancelled/failed, or verification failed — order is NOT placed.
